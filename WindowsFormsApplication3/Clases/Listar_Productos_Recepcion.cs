...
{
    class Listar_Productos_Recepcion
    {
        ...
        public List<Listar_Productos_Recepcion> listarpro()
        {
            //Para almacenar el resultado de la lectura de los datos
            SqlDataReader dr;

            SqlCommand cmd = new SqlCommand("SP_LISTAR_PRODUCTOS_SERVICIOS_FACTURA_RECEPCION", conexion.con);
            ...
            List<Listar_Productos_Recepcion> lstEspe = new List<Listar_Productos_Recepcion>();
            Listar_Productos_Recepcion objeraza;
            while (dr.Read())
            {
                objeraza = new Listar_Productos_Recepcion();
